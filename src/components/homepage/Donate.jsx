...
const Donate = () => {
  return (
    ...
    <section className='xl:py-20 max-w-sm md:max-w-2xl lg:max-w-4xl xl:max-w-6xl mx-auto container'>
      <div className="py-4 xl:py-8 text-gray-800 text-2xl lg:text-5xl font-bold  capitalize leading-[57.60px] text-center">Donate for the cause</div>
      <Slider {...settings}>
        {cardData.map((card) => (
          <Card
            key={card.id}
...
const Card = ({ imageSrc, heading, buttonText }) => {
    ...
    <Link to={'/make-donation'}>
     <div className="absolute bottom-20 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-orange p-4 rounded-full flex items-center justify-center">
        ...
        <div className="py-12">
          <h2 className="text-xl font-semibold mb-2 text-center font-roboto">{heading}</h2>
        </div>
      </div>
    );
  };
