...
const Header = () => {
  const heading = 'The eyes may be blind, but the heart sees all.';

  return (
    <section className="custom-bg w-full pt-20">
      <div className="lg:py-[30px] text-white flex flex-col-reverse lg:flex-row lg:justify-between items-center max-w-sm md:max-w-2xl lg:max-w-4xl xl:max-w-6xl mx-auto container">
        ...
        <div className="py-8 lg:w-1/2">
          <div className="xl:w-[600px] w-full">
            <p className='text-center text-4xl text-black font-poppins leading-10 lg:text-[50px] font-extrabold lg:leading-[60px]'>{heading}</p>
          </div>
          <div className='flex justify-center lg:justify-start py-6  lg:w-1/2 w-full'>
            <Link to={'/donation-page'}>
              <div className="w-[202.78px] h-[65.78px] px-[42px] py-5 bg-orange-500 rounded-full border border-white justify-center items-center gap-2.5 inline-flex">
                ...
                <div className="text-white font-roboto text-sm font-medium ">Donate Now</div>
              </div>
            </Link>
          </div>
        </div>
        ...
        <div className='py-6 lg:w-1/2'>
          <img className="w-full lg:w-[500px] lg:h-[450px] rounded-tr-[20px] rounded-bl-[20px]" src={image} alt="hero"/>
        </div>
      </div>
    </section>
  );
